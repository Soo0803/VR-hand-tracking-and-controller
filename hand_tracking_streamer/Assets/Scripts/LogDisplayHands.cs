...
public class LogDisplayHands : MonoBehaviour
{
    ...
    private void DisplayLog()
    {
        ...
        // --- NEW: Add Active Slot Data (e.g., Controller Grasp) ---
        // Look for slots named: "left_ctrl", "right_ctrl", or "head"
        // depending on the logSource assigned to this panel (Left/Right/Head)
        string slotKey = logSource.ToLower() + (logSource.ToLower() == "head" ? "" : "_ctrl");
        string slotMsg = LogManager.Instance.GetSlotMessage(logSource, slotKey);
        
        if (!string.IsNullOrEmpty(slotMsg))
        {
            _sb.AppendLine(slotMsg);
